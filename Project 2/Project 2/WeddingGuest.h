...
public:
    ...
    WeddingGuest(const WeddingGuest& copy); // Copy Constructor
    
    const WeddingGuest& operator=(const WeddingGuest& copy); // Assignment Operator
    ...
    //dump func to print stuff
    void dump() const{
        Node* p = head;
        while(p){
            std::cerr << p->first << " " << p->last << " " << p->val << std::endl;
            p = p->next;
        }
    }
...
private:
    struct Node {
        ...
        Node(std::string firstName, std::string lastName, GuestType value) : first(firstName), last(lastName), val(value), next(nullptr), prev(nullptr){}
        Node(std::string firstName, std::string lastName, GuestType value, Node *nextNode, Node *prevNode) : first(firstName), last(lastName), val(value), next(nextNode), prev(prevNode){}
    };
