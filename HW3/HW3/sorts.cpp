...
{
    // COMPLETED -> Implement function similarly to compareSensor
...
{
    // COMPLETED -> Implement function to sort vector based on insertion sort
    
    for(int i = 1; i < s.size(); i++){
        Sensor cur = s[i];
        int loc = i;
        while((loc > 0) && comp(cur, s[loc - 1])){
            //Shift s[loc - 1] to the right
            s[loc] = s[loc - 1];
            loc--;
        }
        //Now, s[loc] is where cur belongs
        s[loc] = cur;
    }
...
int main()
{
    ...
    //random_shuffle(ids.begin(), ids.end());  // from <algorithm> - good for C++ 11 or C++ 14
    //use this if compiling in C++ 17 or later
      ...
      // destruction time.
    {
     vector<Sensor> auxSensors(sensors);
        ...
        //COMPLETED -> Create vector of sensor pointers and set each pointer to point to corresponding sensor in auxSensors
        vector<Sensor*> sensorPtrs;
        for(auto i = auxSensors.begin(); i != auxSensors.end(); i++){
            Sensor* ptr = &*i;  //Pointer points to the address of the dereferenced iterator (which is the address of Sensor obj)
            sensorPtrs.push_back(ptr);
        }

        //COMPLETED -> Sort vector of ptrs using STL sort using compareSensorPtr for ordering
        sort(sensorPtrs.begin(), sensorPtrs.end(), compareSensorPtr);

        //COMPLETED -> Use sorted vector of ptrs to replace each sensor in "sensors" with the Sensors from auxSensors in correct order
